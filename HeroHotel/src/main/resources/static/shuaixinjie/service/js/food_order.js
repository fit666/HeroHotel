$(function() {
	...
	function ajaxCount() {
		$.ajax({
			url : '../ServiceFoodOrderCountServlet',
			type : 'post',
			dataType : 'json',
		...
		$.ajax({
			...
			success : function(obj) {
				...
				$.each(obj, function(i, user) {
					...
					str += "<td>" + user.mealtime + "</td>";
					str += "<td>" + user.foodid + "</td>";
					str += "<td>" + user.fprice + "</td>";
					str += "<td>" + state + "</td>";
	...
	function update(id){
		layui.use('layer',function(){
			...
			layer.open({
				  ...
				  fixed: false, //不固定
				  maxmin: true,
				  content: '../ServiceShowFoodOrderServlet?id='+id 
				});
		});
	}
